    ...
    void Update() { 
        ...
        switch (ToShow)
        {
            case ShowXForm.ShowInvT:  // The Green one
                Matrix4x4 TRS_invT = Matrix4x4.Translate(new Vector3(1.4f*delta, 0f, 0f)) * invT * TRS;
                mat.SetMatrix("MyXformMat", TRS_invT);
                break;

            case ShowXForm.ShowInvRT: // The Red one
                Matrix4x4 TRS_invRT = Matrix4x4.Translate(new Vector3(delta, 0f, -delta)) * invR * invT * TRS;
                mat.SetMatrix("MyXformMat", TRS_invRT);
                break;

            case ShowXForm.ShowInvSRT:  // The White one
                Matrix4x4 TRS_invSRT = Matrix4x4.Translate(new Vector3(0f, 0f, -1.4f * delta)) * invS * invR * invT * TRS;
